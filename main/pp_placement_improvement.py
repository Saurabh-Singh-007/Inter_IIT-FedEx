...
def improvement(uld_info, box_info, uld_placement, alloted_weight, alloted_volume):
    uld_length = uld_info["length"]
    uld_width = uld_info["width"]
    uld_height = uld_info["height"]
    uld_weight_limit = uld_info["weight_limit"]
    ...
    placements = uld_placement
    used_boxes = [box_plac["box_id"] for box_plac in uld_placement]
    left_boxes = box_info[~box_info["id"].isin(used_boxes)]

    for box_plac in uld_placement:
        box_id = box_plac['box_id']
        position = box_plac['placement']
        dimensions = box_plac['dimensions']
        point = {
            "x" : position[0],
            "y" : position[1],
            "z" : position[2]
        ...
        update_height_map(box, point, height_map)
        New_points = [
            {"x": position[0] + box["width"], "y": position[1], "z": position[2]},
            {"x": position[0], "y": position[1] + box["length"], "z": position[2]},
            {"x": position[0], "y": position[1], "z": position[2] + box["height"]},
            {"x": position[0] + box["width"], "y": position[1] + box["length"], "z": position[2]},
            {"x": position[0], "y": position[1] + box["length"], "z": position[2] + box["height"]},
            {"x": position[0] + box["width"], "y": position[1], "z": position[2] + box["height"]}, 
            {"x": position[0] + box["width"], "y": position[1] + box["length"], "z": position[2] + box["height"]}
        ]
        potential_points.extend(New_points)
...
def placement_all_uld(boxes_df_temp, uld_data, num_uld, placements):
    iteration_placement = []
    
    for i in range(num_uld):
        weight_util, volume_util, uld_placement = placements[i]
        uld = uld_data.iloc[i]
        if (uld["id"] == 'U3' or uld["id"] == 'U5' or uld["id"] == 'U6'): 
            weight_util, volume_util, left_boxes, placement = improvement(uld, boxes_df_temp, uld_placement, weight_util, volume_util)
        else : 
            weight_util, volume_util, left_boxes, placement = fit_boxes_to_uld(uld, boxes_df_temp, 0, 0)
        boxes_df_temp = left_boxes
...
def worker_task(iter, packages, uld_sorted, num_uld, placements):
    # Each thread works on a copy of priority_df
    priority_df_temp = packages.copy()
    iter_placement = placement_all_uld(priority_df_temp, uld_sorted, num_uld, placements)
    num_priority, cost = cost_calculation(iter_placement)
    iter_placement.append([num_priority, cost, []])
    return iter_placement, cost

def best_placement_improvement(iterations, packages_df, uld_sorted, num_uld, placements):
    ...
    with multiprocessing.Pool(processes=min(12, iterations)) as pool:
        ...
        results = [pool.apply_async(worker_task, args=(iter, packages.copy(), uld_sorted, num_uld, placements)) for iter in range(iterations)]
